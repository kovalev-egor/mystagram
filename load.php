...
$csrf = explode(':', $_POST['csrf']);

if($csrf[0] . ':' . md5($csrf[0] . $_SESSION['secret']) == $_POST['csrf']) {
    ...
    if(isset($_POST['go'])){
        ...
        if(count($err) == 0){
            ...
            move_uploaded_file($_FILES["filename"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . '/' . $uploadDir . '/' . ($filesNum - 1) . '.jpg');
        } else{
    ...
    header('Location: /');
    exit();
}else echo 'Не удалось отправить форму';
